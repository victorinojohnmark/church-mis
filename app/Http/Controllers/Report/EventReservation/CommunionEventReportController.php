...
class CommunionEventReportController extends Controller
{
    public function index(Request $request)
    {
        ...
        if(!is_null($request->name) || (!is_null($request->communion_start) && !is_null($request->communion_end)) || (!is_null($request->daterange_start) && !is_null($request->daterange_end))) {
            ...
            return view('admin.report.event-reservation.communionlist', [
                'communions' => Communion::where($filters)->latest()->get()
            ]);
